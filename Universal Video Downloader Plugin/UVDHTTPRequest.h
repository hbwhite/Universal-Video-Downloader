...
#import <stdio.h>
#import "UVDHTTPRequestConfig.h"
#import "UVDHTTPRequestDelegate.h"
#import "UVDProgressDelegate.h"
#import "UVDCacheDelegate.h"

@class UVDDataDecompressor;

extern NSString *UVDHTTPRequestVersion;
...
typedef enum _UVDAuthenticationState {
	UVDNoAuthenticationNeededYet = 0,
	UVDHTTPAuthenticationNeeded = 1,
	UVDProxyAuthenticationNeeded = 2
} UVDAuthenticationState;

typedef enum _UVDNetworkErrorType {
    UVDConnectionFailureErrorType = 1,
    UVDRequestTimedOutErrorType = 2,
    UVDAuthenticationErrorType = 3,
    UVDRequestCancelledErrorType = 4,
    UVDUnableToCreateRequestErrorType = 5,
    UVDInternalErrorWhileBuildingRequestType  = 6,
    UVDInternalErrorWhileApplyingCredentialsType  = 7,
	UVDFileManagementError = 8,
	UVDTooMuchRedirectionErrorType = 9,
	UVDUnhandledExceptionError = 10,
	UVDCompressionError = 11
	
} UVDNetworkErrorType;


// The error domain that all errors generated by UVDHTTPRequest use
...
extern unsigned long const UVDWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^UVDBasicBlock)(void);
typedef void (^UVDHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^UVDSizeBlock)(long long size);
typedef void (^UVDProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^UVDDataBlock)(NSData *data);
#endif

@interface UVDHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the UVDHTTPRequestDelegate protocol
	id <UVDHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but UVDNetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <UVDHTTPRequestDelegate, UVDProgressDelegate> queue;
	...
	// Automatically set to true in UVDFormDataRequests when using setFile:forKey:
	...
	// Set to true when UVDHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = UVDConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <UVDProgressDelegate> uploadProgressDelegate;
	...
	id <UVDProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force UVDHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, UVDHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, UVDHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - UVDHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	UVDHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in UVDNetworkQueue.h
	...
	// Tells UVDHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let UVDHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// UVDHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See UVDAuthenticationState values above. 0 == default == No authentication needed yet
	UVDAuthenticationState authenticationNeeded;
	
	// When YES, UVDHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops UVDHTTPRequest cleaning up a temporary post body)
	...
	// Will be UVDHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [UVDHTTPRequest setDefaultCache:cache] to configure a default cache
	id <UVDCacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See UVDCacheDelegate.h for possible values
	UVDCachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See UVDCacheDelegate.h for possible values
	UVDCacheStoragePolicy cacheStoragePolicy;
	...
	UVDDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using UVDHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
...
// Used internally to record bandwidth use, and by UVDInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, UVDHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) UVDHTTPRequest *mainRequest;
...
@property (assign, readonly) UVDAuthenticationState authenticationNeeded;
...
@property (assign) id <UVDCacheDelegate> downloadCache;
@property (assign) UVDCachePolicy cachePolicy;
@property (assign) UVDCacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) UVDDataDecompressor *dataDecompressor;
