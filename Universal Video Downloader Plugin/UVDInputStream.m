...
@implementation UVDInputStream

+ (void)initialize
{
	if (self == [UVDInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(UVDHTTPRequest *)theRequest
{
	UVDInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(UVDHTTPRequest *)theRequest
{
	UVDInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask UVDHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([UVDHTTPRequest isBandwidthThrottled]) {
		toRead = [UVDHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[UVDHTTPRequest incrementBandwidthUsedInLastSecond:rv];
	return rv;
}
