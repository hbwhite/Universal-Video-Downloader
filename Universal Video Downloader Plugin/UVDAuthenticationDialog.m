...
static UVDAuthenticationDialog *sharedDialog = nil;
...
@implementation UVDAutorotatingViewController
...
@interface UVDAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation UVDAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [UVDAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(UVDHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		if ([theRequest authenticationNeeded] == UVDProxyAuthenticationNeeded) {
			[sharedDialog setType:UVDProxyAuthenticationType];
		} else {
			[sharedDialog setType:UVDStandardAuthenticationType];
		}
		[sharedDialog show];
	} else {
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[UVDAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
+ (void)dismiss
{
	...
	if ([sharedDialog respondsToSelector:@selector(presentingViewController)]) {
        UIViewController *presentingViewController = [sharedDialog presentingViewController];
        ...
        UIViewController *parentViewController = [sharedDialog parentViewController];
        if ([parentViewController respondsToSelector:@selector(dismissViewControllerAnimated:completion:)]) {
...
- (void)showTitle
{
	...
	if (UIInterfaceOrientationIsPortrait([[UIDevice currentDevice] orientation])) {
		// Setup the title
		if ([self type] == UVDProxyAuthenticationType) {
...
- (void)show
{
    ...
    // Modified to use iOS 5's new modal view controller functions when possible.
    UIViewController *thePresentingController = [self presentingController];
    if ([thePresentingController respondsToSelector:@selector(presentViewController:animated:completion:)]) {
        [thePresentingController presentViewController:self animated:YES completion:nil];
    }
    else {
        [thePresentingController presentModalViewController:self animated:YES];
    }
}
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (UVDHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (UVDHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		UVDHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
...
- (void)loginWithCredentialsFromDialog:(id)sender
{
	for (UVDHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		...
		// Handle NTLM domains
		NSString *scheme = ([self type] == UVDStandardAuthenticationType) ? [[self request] authenticationScheme] : [[self request] proxyAuthenticationScheme];
